...
void initialise_dataset(const char *path_to_dataset,
                        unsigned int print_samples) {
  ...
  strcat(strcpy(training_labels_loc, path_to_dataset),
         "/train-labels-idx1-ubyte");
  strcat(strcpy(testing_set_loc, path_to_dataset), "/t10k-images-idx3-ubyte");
  strcat(strcpy(testing_labels_loc, path_to_dataset),
...
void load_mnist_training_set(char *path_to_training_set) {
  ...
  fread(training_data_buffer, sizeof(training_data_buffer[0]),
        TRAINING_DATA_BYTES, in_file);
  fclose(in_file);

  uint32_t magic_number =
      (training_data_buffer[0] << 24) + (training_data_buffer[1] << 16) +
      (training_data_buffer[2] << 8) + (training_data_buffer[3]);
  uint32_t set_size =
      (training_data_buffer[4] << 24) + (training_data_buffer[5] << 16) +
      (training_data_buffer[6] << 8) + (training_data_buffer[7]);
  uint32_t x_dim = (training_data_buffer[8] << 24) +
                   (training_data_buffer[9] << 16) +
                   (training_data_buffer[10] << 8) + (training_data_buffer[11]);
  uint32_t y_dim = (training_data_buffer[12] << 24) +
                   (training_data_buffer[13] << 16) +
                   (training_data_buffer[14] << 8) + (training_data_buffer[15]);
...
void load_mnist_training_labels(char *path_to_training_set_labels) {
  ...
  fread(training_labels_buffer, sizeof(training_labels_buffer[0]),
        TRAINING_LABELS_BYTES, in_file);
  fclose(in_file);

  // Read headers
  uint32_t magic_number =
      (training_labels_buffer[0] << 24) + (training_labels_buffer[1] << 16) +
      (training_labels_buffer[2] << 8) + (training_labels_buffer[3]);
  uint32_t set_size =
      (training_labels_buffer[4] << 24) + (training_labels_buffer[5] << 16) +
      (training_labels_buffer[6] << 8) + (training_labels_buffer[7]);
  printf("Magic number: %u\n", magic_number);
  printf("Set size: %u\n", set_size);
...
void load_mnist_testing_set(char *path_to_testing_set) {
  ...
  fread(testing_data_buffer, sizeof(testing_data_buffer[0]), TESTING_DATA_BYTES,
        in_file);
  fclose(in_file);

  // Read headers
  uint32_t magic_number =
      (testing_data_buffer[0] << 24) + (testing_data_buffer[1] << 16) +
      (testing_data_buffer[2] << 8) + (testing_data_buffer[3]);
  uint32_t set_size = (testing_data_buffer[4] << 24) +
                      (testing_data_buffer[5] << 16) +
                      (testing_data_buffer[6] << 8) + (testing_data_buffer[7]);
  uint32_t x_dim = (testing_data_buffer[8] << 24) +
                   (testing_data_buffer[9] << 16) +
                   (testing_data_buffer[10] << 8) + (testing_data_buffer[11]);
  uint32_t y_dim = (testing_data_buffer[12] << 24) +
                   (testing_data_buffer[13] << 16) +
                   (testing_data_buffer[14] << 8) + (testing_data_buffer[15]);
  printf("Magic number: %u\n", magic_number);
  printf("Set size: %u\n", set_size);
  printf("x_dim: %u\n", x_dim);
  printf("y_dim: %u\n", y_dim);
...
void load_mnist_testing_labels(char *path_to_testing_set_labels) {
  ...
  fread(testing_labels_buffer, sizeof(testing_labels_buffer[0]),
        TESTING_LABELS_BYTES, in_file);
  fclose(in_file);

  // Read headers
  uint32_t magic_number =
      (testing_labels_buffer[0] << 24) + (testing_labels_buffer[1] << 16) +
      (testing_labels_buffer[2] << 8) + (testing_labels_buffer[3]);
  uint32_t set_size =
      (testing_labels_buffer[4] << 24) + (testing_labels_buffer[5] << 16) +
      (testing_labels_buffer[6] << 8) + (testing_labels_buffer[7]);
  printf("Magic number: %u\n", magic_number);
  printf("Set size: %u\n", set_size);
