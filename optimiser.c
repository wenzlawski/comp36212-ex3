...
void print_training_stats(unsigned int epoch_counter, unsigned int total_iter,
                          double mean_loss, double test_accuracy);

// Optimisation parameters
unsigned int log_freq =
    30000; // Compute and print accuracy every log_freq iterations
...
void initialise_optimiser(double cmd_line_learning_rate,
                          int cmd_line_batch_size, int cmd_line_total_epochs) {
...
void run_optimisation(void) {
  ...
  for (int i = 0; i < num_batches; i++) {
    for (int j = 0; j < batch_size; j++) {
      ...
      if (total_iter % log_freq == 0 || total_iter == 0) {
        ...
        print_training_stats(epoch_counter, total_iter, mean_loss,
                             test_accuracy);
      ...
      // Update iteration counters (reset at end of training set to allow
      // multiple epochs)
      total_iter++;
      training_sample++;
      // On epoch completion:
  ...
  test_accuracy = evaluate_testing_accuracy();
  print_training_stats(epoch_counter, total_iter,
                       (mean_loss / ((double)log_freq)), test_accuracy);
}
