...
// ******** Intermediate gradients (internal to neural network.c - avoids
// re-evaluation during backward pass) ********
...
void inline double_matrix_multiply(unsigned int rows1, unsigned int cols1,
                                   double mat1[][cols1], unsigned int rows2,
                                   unsigned int cols2, double mat2[][cols2],
                                   unsigned int rows_out, unsigned int cols_out,
                                   double mat_out[][cols_out]);
...
void inline evaluate_dh_dW_Lh_Lprev(unsigned int n_h, unsigned int n_prev,
                                    n_ReLu neur[n_h], unsigned int n_W,
                                    double J[][n_W]);
void inline evaluate_dh_dh_prev_inc_pre_act(unsigned int n_h,
                                            unsigned int n_prev,
                                            n_ReLu n_Lh[n_h],
                                            weight_struct_t W[][n_h],
                                            double J[][n_prev]);
void inline evaluate_dh_dW_Lh_Lprev_sparse(unsigned int n_h,
                                           unsigned int n_prev,
                                           n_ReLu neur[n_h], unsigned int n_W,
                                           double J[][n_W]);
void inline sparse_multiply_dL_dh_by_dh_dW_LH_LH_prev(
    unsigned int rows1, unsigned int cols1, double mat1[][cols1], // dl_dh
    unsigned int rows2, unsigned int cols2,
    double mat2[][cols2], // dh_dW_LH_LH_prev
...
double drand(double min, double max) { //
  ...
  random_double =
      (float)rand() /
      RAND_MAX; // note rand() only generates a 32-bit random number (no use
                // casting to double) - could use drand48() to generate doubles
...
void evaluate_dP_dhO(void) {
  for (int i = 0; i < N_NEURONS_LO; i++) {
    for (int j = 0; j < N_NEURONS_LO; j++) {
      // No need to reset term to zero, as every term in Jacobian is to be
      // re-evaluated
      if (i == j) {
...
void inline evaluate_dh_dW_Lh_Lprev(unsigned int n_h, unsigned int n_prev,
                                    n_ReLu neur[n_prev], unsigned int n_W,
                                    double J[][n_W]) {
  for (int i = 0; i < n_h; i++) {
    for (int j = 0; j < (n_W); j++) {
  ...
  for (int i = 0; i < N_NEURONS_L1; i++) {
    for (int j = 0; j < (N_NEURONS_LI * N_NEURONS_L1); j++) {
      if (j >= (i * N_NEURONS_LI) && j < ((i + 1) * N_NEURONS_LI)) {
        dh1_dW_LI_L1[i][j] =
            ((double)training_data[sample][j - (i * N_NEURONS_LI)]) / 255.0;
      } else {
        dh1_dW_LI_L1[i][j] = 0.0;
      }
    }
...
void inline evaluate_dh1_dW_LI_L1_sparse(unsigned int sample) {
  for (int i = 0; i < N_NEURONS_L1; i++) {
    ...
    for (int j = 0; j < (N_NEURONS_LI); j++) {
      dh1_dW_LI_L1[i][start_ind + j] =
          ((double)training_data[sample][j]) / 255.0;
    }
  }
}
...
void inline sparse_multiply_dL_dh_by_dh_dW_LH_LH_prev(
    ...
    double mat2[][cols2], // dh_dW_LH_LH_prev
    unsigned int rows_out, unsigned int cols_out,
    double mat_out[][cols_out]) { // dL_dW_LH_LH_prev
...
void evaluate_forward_pass(uint8_t **dataset, int n) {
  // Forward pass
  // For each layer of the network, j is used to index neurons in current layer,
  // and i to index neurons in preceding layer
  ...
  // Calculate L0 Activations (must calculate all weighted-sum inputs before
  // evaluating softmax)
  for (int j = 0; j < N_NEURONS_LO; j++) {
    n_LO[j].z = 0.0;
    for (int i = 0; i < N_NEURONS_L3; i++) {
...
void evaluate_backward_pass(uint8_t label, unsigned int input_class_index) {
  ...
  // Evaluate derivatives of layerwise activations wrt weight matrix connecting
  // previous layer
  evaluate_dh_dW_Lh_Lprev(N_NEURONS_LO, N_NEURONS_L3, n_L3,
                          N_NEURONS_LO * N_NEURONS_L3,
                          dhO_dW_L3_LO); // dhO_dW_L3_LO
  evaluate_dh_dW_Lh_Lprev(N_NEURONS_L3, N_NEURONS_L2, n_L2,
                          N_NEURONS_L3 * N_NEURONS_L2,
                          dh3_dW_L2_L3); // dh3_dW_L2_L3
  evaluate_dh_dW_Lh_Lprev(N_NEURONS_L2, N_NEURONS_L1, n_L1,
                          N_NEURONS_L2 * N_NEURONS_L1,
                          dh2_dW_L1_L2);    // dh2_dW_L1_L2
  evaluate_dh1_dW_LI_L1(input_class_index); // dh1_dW_LI_L1

  // Evaluate derivatives of layerwise activations wrt activation of previous
  // layer
  evaluate_dh_dh_prev_inc_pre_act(N_NEURONS_LO, N_NEURONS_L3, n_L3, w_L3_LO,
                                  dhO_dh3); // dhO_dh3
  evaluate_dh_dh_prev_inc_pre_act(N_NEURONS_L3, N_NEURONS_L2, n_L2, w_L2_L3,
                                  dh3_dh2); // dh3_dh2
  evaluate_dh_dh_prev_inc_pre_act(N_NEURONS_L2, N_NEURONS_L1, n_L1, w_L1_L2,
                                  dh2_dh1); // dh2_dh1

  // Evaluate Jacobians wrt L
  // dL_dW_L3_LO = (dL_dP * dP_dhO) * dhO_dW_L3_LO
  double_matrix_multiply(1, N_NEURONS_LO, dL_dP, N_NEURONS_LO, N_NEURONS_LO,
                         dP_dhO, 1, N_NEURONS_LO, dL_dhO);
  double_matrix_multiply(1, N_NEURONS_LO, dL_dhO, N_NEURONS_LO,
                         N_NEURONS_L3 * N_NEURONS_LO, dhO_dW_L3_LO, 1,
                         N_NEURONS_L3 * N_NEURONS_LO, dL_dW_L3_LO);

  // dL_dW_L2_L3 = (dL_dhO * dhO_dh3) * dh3_dW_L2_L3
  double_matrix_multiply(1, N_NEURONS_LO, dL_dhO, N_NEURONS_LO, N_NEURONS_L3,
                         dhO_dh3, 1, N_NEURONS_L3, dL_dh3);
  double_matrix_multiply(1, N_NEURONS_L3, dL_dh3, N_NEURONS_L3,
                         N_NEURONS_L2 * N_NEURONS_L3, dh3_dW_L2_L3, 1,
                         N_NEURONS_L2 * N_NEURONS_L3, dL_dW_L2_L3);

  // dL_dW_L1_L2 = (dL_h3 * dh3_dh2) * dh2_dW_L1_L2
  double_matrix_multiply(1, N_NEURONS_L3, dL_dh3, N_NEURONS_L3, N_NEURONS_L2,
                         dh3_dh2, 1, N_NEURONS_L2, dL_dh2);
  double_matrix_multiply(1, N_NEURONS_L2, dL_dh2, N_NEURONS_L2,
                         N_NEURONS_L1 * N_NEURONS_L2, dh2_dW_L1_L2, 1,
                         N_NEURONS_L1 * N_NEURONS_L2, dL_dW_L1_L2);

  // dL_dW_LI_L1 = (dL_dh2 * dh2_dh1) * dh1_dW_LI_L1
  double_matrix_multiply(1, N_NEURONS_L2, dL_dh2, N_NEURONS_L2, N_NEURONS_L1,
                         dh2_dh1, 1, N_NEURONS_L1, dL_dh1);
  double_matrix_multiply(1, N_NEURONS_L1, dL_dh1, N_NEURONS_L1,
                         N_NEURONS_LI * N_NEURONS_L1, dh1_dW_LI_L1, 1,
                         N_NEURONS_LI * N_NEURONS_L1, dL_dW_LI_L1);
}

void evaluate_backward_pass_sparse(uint8_t label,
                                   unsigned int input_class_index) {
  ...
  evaluate_dh_dW_Lh_Lprev_sparse(N_NEURONS_LO, N_NEURONS_L3, n_L3,
                                 N_NEURONS_LO * N_NEURONS_L3,
                                 dhO_dW_L3_LO); // dhO_dW_L3_LO
  evaluate_dh_dW_Lh_Lprev_sparse(N_NEURONS_L3, N_NEURONS_L2, n_L2,
                                 N_NEURONS_L3 * N_NEURONS_L2,
                                 dh3_dW_L2_L3); // dh3_dW_L2_L3
  evaluate_dh_dW_Lh_Lprev_sparse(N_NEURONS_L2, N_NEURONS_L1, n_L1,
                                 N_NEURONS_L2 * N_NEURONS_L1,
                                 dh2_dW_L1_L2);    // dh2_dW_L1_L2
  ...
  double_matrix_multiply(1, N_NEURONS_LO, dL_dP, N_NEURONS_LO, N_NEURONS_LO,
                         dP_dhO, 1, N_NEURONS_LO, dL_dhO);
  sparse_multiply_dL_dh_by_dh_dW_LH_LH_prev(
      1, N_NEURONS_LO, dL_dhO, N_NEURONS_LO, N_NEURONS_L3 * N_NEURONS_LO,
      dhO_dW_L3_LO, 1, N_NEURONS_L3 * N_NEURONS_LO, dL_dW_L3_LO);

  // dL_dW_L2_L3 = (dL_dhO * dhO_dh3) * dh3_dW_L2_L3
  double_matrix_multiply(1, N_NEURONS_LO, dL_dhO, N_NEURONS_LO, N_NEURONS_L3,
                         dhO_dh3, 1, N_NEURONS_L3, dL_dh3);
  sparse_multiply_dL_dh_by_dh_dW_LH_LH_prev(
      1, N_NEURONS_L3, dL_dh3, N_NEURONS_L3, N_NEURONS_L2 * N_NEURONS_L3,
      dh3_dW_L2_L3, 1, N_NEURONS_L2 * N_NEURONS_L3, dL_dW_L2_L3);

  // dL_dW_L1_L2 = (dL_h3 * dh3_dh2) * dh2_dW_L1_L2
  double_matrix_multiply(1, N_NEURONS_L3, dL_dh3, N_NEURONS_L3, N_NEURONS_L2,
                         dh3_dh2, 1, N_NEURONS_L2, dL_dh2);
  sparse_multiply_dL_dh_by_dh_dW_LH_LH_prev(
      1, N_NEURONS_L2, dL_dh2, N_NEURONS_L2, N_NEURONS_L1 * N_NEURONS_L2,
      dh2_dW_L1_L2, 1, N_NEURONS_L1 * N_NEURONS_L2, dL_dW_L1_L2);

  // dL_dW_LI_L1 = (dL_dh2 * dh2_dh1) * dh1_dW_LI_L1
  double_matrix_multiply(1, N_NEURONS_L2, dL_dh2, N_NEURONS_L2, N_NEURONS_L1,
                         dh2_dh1, 1, N_NEURONS_L1, dL_dh1);
  sparse_multiply_dL_dh_by_dh_dW_LH_LH_prev(
      1, N_NEURONS_L1, dL_dh1, N_NEURONS_L1, N_NEURONS_LI * N_NEURONS_L1,
      dh1_dW_LI_L1, 1, N_NEURONS_LI * N_NEURONS_L1, dL_dW_LI_L1);
}
